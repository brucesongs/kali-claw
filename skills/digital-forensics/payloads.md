...
## 19. Disk Imaging Verification

### Multi-Algorithm Hash Verification

```bash
# Generate multi-algorithm hashes for disk image verification
sha256sum evidence.dd > evidence.hashes
md5sum evidence.dd >> evidence.hashes
sha1sum evidence.dd >> evidence.hashes

# Verify all hashes against stored values
echo "=== Verifying SHA-256 ===" && sha256sum -c evidence.hashes --quiet
echo "=== Verifying MD5 ===" && md5sum -c evidence.hashes --quiet
echo "=== Verifying SHA-1 ===" && sha1sum -c evidence.hashes --quiet

# Verify E01 image integrity with ewfinfo
ewfinfo evidence.E01 | grep -E "MD5|SHA1|SHA256|bytes"
ewfverify evidence.E01

# Segment-level verification for split images
for seg in evidence.E01 evidence.E02 evidence.E03; do
  echo "Verifying $seg..."
  ewfverify "$seg" 2>&1 | grep -E "MD5|integrity"
done
```

---

## 20. Memory Analysis Scripts

### Automated Memory Forensics Pipeline

```bash
#!/bin/bash
# Automated Volatility3 analysis pipeline for Windows memory images
IMAGE="${1:-memory.raw}"
OUTDIR="forensics-output/$(date +%Y%m%d_%H%M%S)"
mkdir -p "$OUTDIR"

echo "[*] Running full Volatility3 pipeline on $IMAGE"

echo "[1/8] Process list..." && vol3 -f "$IMAGE" windows.pslist.PsList > "$OUTDIR/pslist.txt" 2>/dev/null
echo "[2/8] Process tree..." && vol3 -f "$IMAGE" windows.pstree.PsTree > "$OUTDIR/pstree.txt" 2>/dev/null
echo "[3/8] Hidden process scan..." && vol3 -f "$IMAGE" windows.psscan.PsScan > "$OUTDIR/psscan.txt" 2>/dev/null
echo "[4/8] Network connections..." && vol3 -f "$IMAGE" windows.netscan.NetScan > "$OUTDIR/netscan.txt" 2>/dev/null
echo "[5/8] Malware detection..." && vol3 -f "$IMAGE" windows.malfind.Malfind > "$OUTDIR/malfind.txt" 2>/dev/null
echo "[6/8] DLL list..." && vol3 -f "$IMAGE" windows.dlllist.DllList > "$OUTDIR/dlllist.txt" 2>/dev/null
echo "[7/8] Command lines..." && vol3 -f "$IMAGE" windows.cmdline.CmdLine > "$OUTDIR/cmdline.txt" 2>/dev/null
echo "[8/8] Registry hives..." && vol3 -f "$IMAGE" windows.registry.hivelist.HiveList > "$OUTDIR/hivelist.txt" 2>/dev/null

# Summary
echo ""
echo "=== Analysis Complete ==="
echo "Total processes: $(wc -l < "$OUTDIR/pslist.txt")"
echo "Network connections: $(wc -l < "$OUTDIR/netscan.txt")"
echo "Suspicious injections: $(grep -c "^0x" "$OUTDIR/malfind.txt" 2>/dev/null || echo 0)"
...
```bash
# Generate a unified forensic timeline from disk, memory, and log sources
TIMELINE_DIR="timeline-output"
mkdir -p "$TIMELINE_DIR"

# Step 1: Filesystem MAC timeline (SleuthKit)
echo "[1/4] Generating filesystem timeline..."
fls -r -m "/" -o 2048 evidence.dd > "$TIMELINE_DIR/fs_body.txt"
mactime -b "$TIMELINE_DIR/fs_body.txt" -d > "$TIMELINE_DIR/fs_timeline.csv"

# Step 2: Windows Event Log timeline
echo "[2/4] Parsing Windows Event Logs..."
evtx_dump.py Security.evtx 2>/dev/null | \
  grep -oP '\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}' | sort > "$TIMELINE_DIR/evt_times.txt"

# Step 3: Network capture timestamps
echo "[3/4] Extracting network capture timestamps..."
tshark -r capture.pcap -T fields -e frame.time_epoch -e ip.src -e ip.dst -e tcp.dstport 2>/dev/null | \
  awk -F'\t' '{print strftime("%Y-%m-%d %H:%M:%S",int($1)),",NETWORK,",$2," -> ",$3":"$4}' \
  > "$TIMELINE_DIR/net_timeline.csv"

# Step 4: Merge all timelines
echo "[4/4] Merging timelines..."
cat "$TIMELINE_DIR/fs_timeline.csv" "$TIMELINE_DIR/net_timeline.csv" | \
  sort -t',' -k1 > "$TIMELINE_DIR/master_timeline.csv"

echo "Master timeline: $TIMELINE_DIR/master_timeline.csv"
wc -l "$TIMELINE_DIR/master_timeline.csv"
```

### Evidence Hash Chain Script

```bash
#!/bin/bash
# Maintain cryptographic hash chain of custody for all evidence files
# Run after each analysis step to detect tampering

EVIDENCE_DIR="${1:-./evidence}"
HASH_FILE="evidence_chain_$(date +%Y%m%d_%H%M%S).txt"

echo "# Evidence Chain of Custody - $(date -u '+%Y-%m-%dT%H:%M:%SZ')" > "$HASH_FILE"
echo "# Generated by: $(whoami)@$(hostname)" >> "$HASH_FILE"
echo "# Directory: $EVIDENCE_DIR" >> "$HASH_FILE"
echo "---" >> "$HASH_FILE"

find "$EVIDENCE_DIR" -type f -not -name "*.txt" | sort | while read -r file; do
  sha256=$(sha256sum "$file" | awk '{print $1}')
  md5=$(md5sum "$file" | awk '{print $1}')
  size=$(stat -f%z "$file" 2>/dev/null || stat -c%s "$file" 2>/dev/null)
  modified=$(stat -f%Sm -t '%Y-%m-%dT%H:%M:%S' "$file" 2>/dev/null || stat -c%y "$file" 2>/dev/null)
  echo "$sha256  $md5  $size  $modified  $file" >> "$HASH_FILE"
done

echo "Hash chain recorded: $HASH_FILE"
echo "Total files: $(grep -c '^.' "$HASH_FILE" | tail -1)"

# Verify against previous chain
PREV="${2:-none}"
if [ -f "$PREV" ]; then
  echo "=== Comparing with previous chain ==="
  while IFS= read -r line; do
    [ "$line" = "---" ] && continue
    [[ "$line" = \#* ]] && continue
    current_sha=$(echo "$line" | awk '{print $1}')
    file=$(echo "$line" | awk '{print $NF}')
    prev_sha=$(grep "  $file$" "$PREV" | awk '{print $1}')
