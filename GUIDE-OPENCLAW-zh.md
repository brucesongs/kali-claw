...
> 面向 v0.1.39 | 从零开始，手把手教你搭建和使用 AI 渗透测试智能体（137 个技能域、33 个 Distinguished）
...
- **137 个安全技能域** — 覆盖 Web 安全、网络渗透、密码攻击、云与身份、容器与云原生、AI/LLM 安全、企业身份（Entra ID/Okta）、现代企业栈（PAM/CI-CD/CSPM-CASB/SASE-SSE）、关键基础设施（SCADA/ICS）、区块链/Web3、移动与物联网、量子与后量子、无线电/卫星等
...
这会生成所有 137 个技能域的 JSON 评分文件到 `validation/evidence/quality-scores/`。如果你计划自己改造技能，这套分数能帮你定位薄弱环节。
...
kali-claw 拥有 **137 个技能域**，按攻击面/防御面/能力类型大致分为 8 大类：
...
```bash
bash validation/SCORE.sh                # 全量评分 137 个技能
...
| 阶段 | 版本 | 关键里程碑 |
|------|------|------------|
| 基础建设 | v0.1.1 - v0.1.7 | 45 → 137 个技能域，建立分层架构 |
...
**平均分：88.78** | **137/137 Excellent+（100%）** | **33 Distinguished**
...
| `skills/` | 137 个技能域目录 | 添加新技能 / 更新现有技能 |
...
_Built with the OpenClaw Agent Framework. 当前版本 v0.1.39（137 个技能域、33 Distinguished）。如有问题，请在 GitHub Issues 中反馈。_
